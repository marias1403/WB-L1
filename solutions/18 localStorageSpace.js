// Условие. Подсчитать максимальный объем данных, который можно записать в localStorage вашего браузера.

// Решение.
// В веб-браузерах нет прямого способа определить максимальный объем localStorage через JavaScript без итеративной проверки.
// Это связано с тем, что спецификация Web Storage не предоставляет методов для получения точного максимального размера хранилища
function getMaxLocalStorageSize() {
  ...
  let totalSize = 0;
  // Выполняем код внутри блока try {...} catch (e) {...}, ошибки будут обрабатываться в блоке catch
  try {
